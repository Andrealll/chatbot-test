...
# === Importa in modo difensivo dal core ===
try:
...
def _normalize_angles(raw: Dict[str, Any]) -> Dict[str, float]:
    """
    Converte un dict potenzialmente annidato in { nome: gradi_float % 360 }.
...
# Rileva dinamicamente se la firma supporta 'colonne_extra'
...
def _angles_at(when: datetime) -> Dict[str, float]:
    ...
    if _HAS_COLONNE_EXTRA:
        raw = calcola_pianeti_da_df(df_tutti, when.day, when.month, when.year, colonne_extra=("Nodo", "Lilith"))
    else:
        raw = calcola_pianeti_da_df(df_tutti, when.day, when.month, when.year)
    return _normalize_angles(raw)

# === Intensita' mock (fallback deterministico) ===
...
def _mock_intensities_from_angles(angles: Dict[str, float]) -> Dict[str, float]:
    ...
    f1 = float(np.mean(v % 30.0) / 30.0)
    f2 = float(np.clip(np.std(v) / 180.0, 0.0, 1.0))
    f3 = float((np.mean(np.cos(np.radians(v))) + 1.0) / 2.0)
    f4 = float((np.mean(np.sin(np.radians(v))) + 1.0) / 2.0)
...
def compute_oroscopo_daily(when: datetime, place: Place, natal: Natal, png_path: Path) -> Dict[str, Any]:
    ...
    intensities = None
    # Se hai funzioni reali di transiti rispetto al natal, chiamale qui
    try:
...
def compute_oroscopo_period(samples: List[datetime], place: Place, natal: Natal, png_path: Path) -> Dict[str, Any]:
    ...
    return {"intensities": agg, "n_samples": len(chunks)}
