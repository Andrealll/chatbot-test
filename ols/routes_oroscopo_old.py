...
# Costi in crediti per oroscopo premium per periodo (PARAMETRICI)
OROSCOPO_FEATURE_KEY_PREFIX = "oroscopo"
    ...
    if periodo.startswith("giorn"):
        if tier == "premium":
            ...
            out = [
                ...
                {"id": "pomeriggio", "label": "Pomerigiorno", "datetime": start_dt.replace(hour=15).isoformat()},
...
async def oroscopo_macro(
    ...
    # ===========================
    # GATING CREDITI (solo premium + engine=ai)
    # ===========================
    state = None
    decision = None
    feature_cost = 0
                ...
                if tier == "premium":
                    ...
                    if state is not None and decision is not None:
                        ...
                        if feature_cost > 0 and mode == "paid":
                            ...
                            # premium ma free_try / trial: costo economico = 0
                            log_usage_event(
                                user_id=user.sub,
                                feature=feature_name,
                                cost_credits=0,
                        ...
                        # premium ma senza gating (es. engine != "ai")
                        log_usage_event(
                            user_id=user.sub,
                            feature=feature_name,
                            cost_credits=0,
                    ...
                    # tier free: tracciamo comportamento, costo=0
                    log_usage_event(
                        user_id=user.sub,
                        feature=feature_name,
                        cost_credits=0,
