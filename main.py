...
async def tema(payload: TemaRequest):
    ...
    try:
        ...
        # Costruisco un dizionario tema che useremo eventualmente per la firma vecchia della carta
        dati_tema = {
        ...
        # 4) Grafico polare (prova firma nuova, poi fallback)
        try:
            try:
                # Firma nuova (esplicita, con tanti parametri)
        ...
        # 5) Interpretazione Groq (prova firma nuova, poi fallback)
        try:
            try:
                # Firma nuova
                ...
                # Firma vecchia ipotizzata: (pianeti_decod, asc_mc_case, domanda, scope)
                interpretazione = interpreta_groq(
                    pianeti_decod,
                    asc_mc_case,
                    payload.domanda,
                    payload.scope or "tema",
                )
        except Exception as e:
        ...
        # validazione ecc.: rilanciamo così FastAPI risponde con detail
        raise
    except Exception as e:
        # errore generico /tema
